...
extern "C"
{
	...
	AI_API void Update()
	{
		...
		tank.fireDistance = (int)sqrt((float)((tank.x - enemyX) * (tank.x - enemyX) + (tank.y - enemyY) * (tank.y - enemyY))) / 1;
		tank.fireAngle = -(int)atan2((float)(enemyY - tank.y), (float)(tank.x - enemyX)) * 180 / M_PI - 90;
